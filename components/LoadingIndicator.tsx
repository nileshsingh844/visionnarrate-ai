...
const LoadingIndicator: React.FC<LoadingIndicatorProps> = ({ customStatus, progress = 0, state }) => {
  return (
    <div className="flex flex-col items-center justify-center p-16 md:p-24 bg-[#050505] rounded-[48px] border border-white/5 shadow-2xl w-full max-w-2xl relative overflow-hidden group">
      ...
      <div className="mt-12 grid grid-cols-2 gap-4 w-full opacity-40 hover:opacity-100 transition-opacity">
        <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
           <p className="text-[8px] font-bold text-slate-600 uppercase mb-1">Context_In</p>
           <p className="text-[10px] text-slate-300 font-mono">1.28 GB Ground Truth</p>
        </div>
        <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
           <p className="text-[8px] font-bold text-slate-600 uppercase mb-1">Compute_Alloc</p>
           <p className="text-[10px] text-slate-300 font-mono">Gemini 3 Pro Cluster</p>
        </div>
      </div>
    </div>
  );
